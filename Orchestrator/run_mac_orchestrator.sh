...
if [ ! -d ".venv" ]; then
    ...
    if [[ $confirm == [sS] || $confirm == [yY] ]]; then
        ...
        if [[ "$OS" == "Linux" ]] && command -v nvidia-smi &> /dev/null; then
            ...
            .venv/bin/pip install torch==2.4.1 --index-url https://download.pytorch.org/whl/cu121  # Changed to specific version 2.4.1
        else
            echo "💻 Installing standard PyTorch 2.4.1..."
            .venv/bin/pip install torch==2.4.1  # Changed to specific version 2.4.1
        fi
    else
...
if [[ "$OS" == "Linux" ]] && command -v nvidia-smi &> /dev/null; then
    ...
    if [[ "$HAS_CUDA" == "False" ]]; then
        ...
        if [[ $recnfirm == [sS] || $recnfirm == [yY] ]]; then
            .venv/bin/pip install --force-reinstall torch==2.4.1 --index-url https://download.pytorch.org/whl/cu121  # Changed to specific version 2.4.1
        fi
    fi
fi
...
# 6. Activate the virtual environment (this is the key fix)
